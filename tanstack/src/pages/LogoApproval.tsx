...
import { Card, Button, Spin, message, Empty, Modal, Input, Image, Tag, Space } from 'antd';
...
const LogoApproval: React.FC = () => {
    ...
    const [selectedSponsor, setSelectedSponsor] = useState<SponsorEntry | null>(null);
    ...
    const rejectMutation = useMutation({
        mutationFn: ({ sponsorshipId, reason }: { sponsorshipId: string; reason: string }) =>
            sponsorshipService.approveLogo(sponsorshipId, { approved: false, rejectionReason: reason }),
        onSuccess: () => {
    ...
    const handleRejectConfirm = () => {
        ...
        rejectMutation.mutate({ sponsorshipId: selectedSponsor._id, reason: rejectionReason });
    };
        ...
        <div style={{ padding: 'clamp(12px, 3vw, 24px)', maxWidth: 1200, margin: '0 auto' }}>
            <div style={{ marginBottom: 24 }}>
                <Button
                    ...
                    onClick={() => navigate({ to: '/campaigns/$id', params: { id: id! } })}
                ...
                <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
                    {pendingLogos.map((sponsor) => (
                        ...
                        >
                            ...
                            <p><strong>Amount:</strong> ${sponsor.amount} {campaign.currency}</p>
                            {sponsor.message && <p><strong>Message:</strong> {sponsor.message}</p>}
